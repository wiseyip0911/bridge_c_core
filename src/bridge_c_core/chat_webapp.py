...
- **可选换肤**: 若设置 ``BRIDGE_C_CHAT_WEB_STATIC_DIR`` 指向某目录,则 ``GET /``
  从该目录加载 ``index.html``(适配器可自带品牌静态页,不必 fork FastAPI 逻辑)。
- **不强加 schema**: ``/api/send`` 的 body 只是简单转换成 ``submit_to``,
  ``payload_json.channel`` 默认 ``lookup``(与 ``docs/HERMES.md`` 约定一致:
  查询/能力优先;纯闲聊请显式选 ``chat``)。
...
ENV_STATIC_DIR = "BRIDGE_C_CHAT_WEB_STATIC_DIR"
...
def _static_dir() -> Path:
    """静态页目录:``BRIDGE_C_CHAT_WEB_STATIC_DIR`` 优先,否则用包内 ``web_static``。"""
    raw = os.environ.get(ENV_STATIC_DIR, "").strip()
    if raw:
        p = Path(raw)
        return p if p.is_absolute() else (Path.cwd() / p).resolve()
...
class SendBody(BaseModel):
    ...
    channel: str = Field(default="lookup", min_length=1)
